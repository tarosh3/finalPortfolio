...
export type SideItem = { icon: string; label: string; active?: boolean; onClick?: () => void; badge?: string };
      ...
      <nav className="os-side__nav">
        {items.map((it, i) => (
          <button
            key={i}
            type="button"
            className={`os-side__item ${it.active ? "is-active" : ""}`}
            onClick={it.onClick}
            aria-pressed={it.active}
          >
            <Sym name={it.icon} size={20} fill={it.active} />
            {it.label}
            {it.badge && <span className="os-side__badge">{it.badge}</span>}
          </button>
        ))}
      </nav>
